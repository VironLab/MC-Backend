...
class PacketIsReplayService() : JsonPacket() {

    constructor(serviceId: UUID) : this() {
        this.jsonLib = this.jsonLib.append("service", serviceId)
    }

    override suspend fun handle(connection: IConnection): ICommunicationPromise<BooleanConfiguration> {
        val serviceId = this.jsonLib.getObject("service", UUID::class.java) ?: return contentException("service")
        return CommunicationPromise.of(
            ManagerPacketModerationConstant.moderationFeature.replayServices.containsKey(
                serviceId
            ).toConfig()
        )
    }
